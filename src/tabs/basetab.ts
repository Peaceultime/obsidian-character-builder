...
export class BaseTab extends Tab
{
	render()
	{
		...
		const armorSlider = new Slider(splitElmt, `Armure max`).desc(`L'armure maximum determine le nombre de talents disponibles au niveau 1.`).range(1, 3, 1).value(1);
		...
		armorSlider.onChange(value => {
			...
			metadata.talents = 6 - value;

			talentText.value(6 - value);
		}).value(metadata.armor ?? 1).tooltip(true);
